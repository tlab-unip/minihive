...
class TestRuleBreakUpSelections(unittest.TestCase):
    ...
    def _check(self, input, expected):
        ...
        self.assertIsInstance(computed_expr, radb.ast.Node)
...
class TestRuleMergeSelections(unittest.TestCase):
...
class TestRuleIntroduceJoins(unittest.TestCase):
    ...
    def test_extend_1(self):
        self._check("""\select_{Eats.pizza = Serves.pizza}((\select_{Person.name = Eats.name}
                       (\select_{Person.name = 'Amy'} Person \cross \select_{Eats.name = 'Amy'} Eats)) \cross \select_{Serves.pizzeria = 'Pizza Hut'} Serves);""",
                    """(\select_{Person.name = 'Amy'} Person \join_{Person.name = Eats.name} \select_{Eats.name = 'Amy'} Eats) \join_{Eats.pizza =
...
class TestAllStepsDifferentSchema(unittest.TestCase):
    ...
    def _check(self, input, expected):
        dd = {}
        dd["Employee"] = {"id": "integer", "name": "string", "age": "integer", "gender": "string", "actor": "string"}
        dd["Company"] = {"id": "integer", "name": "string"}
        dd["Interest"] = {"id": "integer", "name": "string"}
        dd["interested_in"] = {"emp_id": "integer", "int_id": "integer"}
        dd["works_at"] = {"emp_id": "integer", "com_id": "integer"}
    ...
    def test_crowd_1(self):
        self._check("\select_{Employee.id = works_at.emp_id} (Employee \cross works_at);",
                    "Employee \join_{Employee.id = works_at.emp_id} works_at;")

    def test_crowd_2(self):
        self._check(
            "\select_{Employee.id = works_at.emp_id and Employee.name = 'Moss'} (works_at \cross Employee);",
            "works_at \join_{Employee.id = works_at.emp_id} \select_{Employee.name = 'Moss'} Employee;")

    def test_crowd_3(self):
        self._check("""\project_{Employee.name} \select_{Employee.id = works_at.emp_id and works_at.com_id = Company.id}
                       ((Employee \cross works_at) \cross Company);""",
                    """\project_{Employee.name} ((Employee \join_{Employee.id = works_at.emp_id} works_at) 
                       \join_{works_at.com_id = Company.id} Company);""")

    def test_crowd_4(self):
        self._check("""\project_{E.name, W.emp_id} (\select_{E.id = W.emp_id}
                       ((\\rename_{E: *} Employee) \cross (\\rename_{W: *} works_at)));""",
                    """\project_{E.name, W.emp_id} ((\\rename_{E: *} Employee) \join_{E.id = W.emp_id}
                       (\\rename_{W: *} works_at));""")
